...
| 2026-06-03 18:30 | claude (lab-reader) | 🔒 deploying — **in-app Living Lab reader**: `/feed` + `/feed/<date>` now render blog-style human-readable discoveries in the SPA (over live /feed.json) instead of bouncing to home; entry link on the daily Stats page. Frontend-only (new `public/feed.js` + `app.js` routes + `style.css`). 342 tests green. | — | 🔒 deploying |
